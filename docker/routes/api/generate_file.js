...
async function create_combine(data){
    csv_writer = new ObjectsToCsv(data)
    await csv_writer.toDisk('./assets/webscrap/styphi/combine.csv')
}
...
router.get('/', function(req, res) {
    ...
    for (file of read_files){
        fs.createReadStream(`./assets/webscrap/styphi/${file}`, {start: 0})
        .pipe(csv())
        .on('data', (data) => {
            ...
            index = data_to_write.indexOf(obj_parser[0])
            new_obj = true
            ...
            if (column_names.indexOf("COUNTRY") != -1){
                ...
                if (COUNTRY.indexOf("travel") != -1 || COUNTRY.indexOf("Travel") != -1){
                    ...
                    obj_parser["COUNTRY_ONLY"] = (COUNTRY=="None" || COUNTRY=="Unknown" || COUNTRY=="") ? "-" : TRAVEL[0].trim()
                ...
                if (COUNTRY.indexOf(":") != -1){
                    ...
                    obj_parser["TRAVEL"] = obj_parser["TRAVEL"] == undefined ? "-" : obj_parser["TRAVEL"]
                    if (REGION_IN_COUNTRY.indexOf("travel") != -1 || REGION_IN_COUNTRY.indexOf("Travel") != -1){
                        ...
                        if (remove_travels.indexOf(TRAVEL[2]) != -1 || TRAVEL[1] == "No travel abroad"){
                            TRAVEL_LOCATION = "-"
                            TRAVEL = "-"
                        }
                        else{
                ...
                for (column of headers_metadata){
                    obj_parser[column] = data[column]
                }
                obj_parser["LOCATION"] = data['COUNTRY']                    
            }
            if (column_names.indexOf("Genome ID") != -1){
                for (column of headers_species_prediction){
                    obj_parser[column] = data[column]
                }
            }
            if (column_names.indexOf('REFERENCE') != -1){
                for (column of headers_typing){
                    obj_parser[column] = data[column]
                }
            }
            if (column_names.indexOf("GENOTYPHI GENOTYPE") != -1){
                h58_genotypes = ["4.3.1", "4.3.1.1", "4.3.1.1.P1", "4.3.1.2", "4.3.1.3"]
                curate_223 =["9953_5_76_LaoLNT1480_2010",
            ...
            if (column_names.indexOf("gyrA_S83F") != -1){
                for (column of headers_amr_snps){
                    obj_parser[column] = data[column]
                }
            ...
            if (column_names.indexOf("PW_AMP") != -1){
                for (column of headers_amr_profile){
                    obj_parser[column] = data[column]
                }
            }
        ...
        .on('end', async () =>{
            await create_combine(data_to_write)
