...
router.get('/:filter1/:country/:min_year/:max_year', function (req, res, next) {
  ...
  let results_json = [];
  let results = [];
  fs.createReadStream(path.join(__dirname, '../..', 'assets/webscrap/styphi/combine.csv'))
    .pipe(csv())
    ...
    .on('end', () => {
      for (data of results_json) {
        ...
        if ((data["COUNTRY_ONLY"] == params.country) && (params.min_year <= data["DATE"] && data["DATE"] <= params.max_year)) {
          if (params.filter1 == "1") {
        ...
        else if ((params.country == "all") && (params.min_year <= data["DATE"] && data["DATE"] <= params.max_year)) {
          if (params.filter1 == "1") {
            filter_value["blaCTX-M-15_23"] = data["blaCTX-M-15_23"]
            results.push(filter_value)
          }
    ...
    .on('end', () => {
      for (data of results) {
        if (params.country != "all") {
          ...
          if ((params.min_year <= data["DATE"] && data["DATE"] <= params.max_year)) {
