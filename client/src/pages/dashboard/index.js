...
const DashboardPage = () => {
  ...
  useEffect(() => {
    const timeOutId = setTimeout(() => {
      axios.get(`${API_ENDPOINT}filters/all/${timePeriodRange[0]}/${timePeriodRange[1]}`)
        .then((res) => {
  ...
  const parseDataForChart = (data) => {
    ...
    if (!arraysEqual(finalFirstChartData, firstChartData))
  ...
  const parseDataForCountryMap = (data) => {
    ...
    if (!arraysEqual(finalCountries, worldMapSamplesData)) {
      ...
      var centralAfricanRepublicCountryIndex = finalCountries.findIndex(e => e.name === 'Central African Republic');
      if (centralAfricanRepublicCountryIndex !== -1)
        finalCountries[centralAfricanRepublicCountryIndex].displayName = 'Central African Rep.'

      var ivoryCoastCountryIndex = finalCountries.findIndex(e => e.name === "Ivory Coast");
      if (ivoryCoastCountryIndex !== -1)
        finalCountries[ivoryCoastCountryIndex].displayName = "Côte d'Ivoire"

      var timorLesteCountryIndex = finalCountries.findIndex(e => e.name === "East Timor");
      if (timorLesteCountryIndex !== -1)
        finalCountries[timorLesteCountryIndex].displayName = "Timor-Leste"
  ...
  return (
    <div className="dashboard">
      ...
      <div className="footer">
        <span>Data obtained from: <a href="https://pathogen.watch" rel="noreferrer" target="_blank">pathogen watch project</a> on 05/11/2020.</span>
