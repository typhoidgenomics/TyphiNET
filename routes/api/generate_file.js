...
router.get('/create', function (req, res) {
    ...
    let headers_metadata = []

    const headers_metadata_new = ["DATE", "TGC SANGERLANE", "ACCESSION", "AGE", "COUNTRY OF ORIGIN", "LOCATION", "COUNTRY ISOLATED", "TRAVEL ASSOCIATED", "TRAVEL COUNTRY", "STRAIN", "PURPOSE OF SAMPLING", "SOURCE",
        "SYMPTOM STATUS", "PROJECT ACCESSION", "BIOSAMPLE", "LAB", "CONTACT", "ACCURACY", "PMID", "LATITUDE", "LONGITUDE"
    ]

    const headers_metadata_old = ["ACCURACY", "STRAIN", "SOURCE", "ACCESSION", "STUDY ACCESSION",
    ...
    for (let file of read_files) {
        ...
        fs.createReadStream(path.join(__dirname, `../../assets/webscrap/raw_data/${file}`), { start: 0 })
            ...
            .on('data', (data) => {
                ...
                if (data_name !== "NA") {
                    ...
                    if (file === "pw_metadata.csv") {
                        ...
                        if (column_names.includes("COUNTRY OF ORIGIN")) {
                            headers_metadata = headers_metadata_new
                        } else {
                            headers_metadata = headers_metadata_old
                        }
                        ...
                        obj_parser["DATE"] = empty.includes(obj_parser["DATE"]) ? "-" : obj_parser["DATE"]
            ...
            .on('end', async () => {
                ...
                let temp = []
                const currentMetadata = Object.keys(data_to_write[0]).includes("COUNTRY OF ORIGIN")
